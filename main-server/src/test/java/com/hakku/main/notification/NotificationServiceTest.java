...
class NotificationServiceTest {
    ...
    void getNotifications_returnsListFromRedis() throws Exception {
        ...
        String json = objectMapper.writeValueAsString(
                new NotificationResponse(NotificationType.COMMENT, 2L, "테스터", 10L, "제목", null, "테스트 댓글", now));
    ...
    void saveNotification_pushesToRedisAndTrims() {
        NotificationEvent event = new NotificationEvent(
                NotificationType.COMMENT, 1L, 2L, "테스터", 10L, "제목", null, "댓글을 달았습니다.", Instant.now().toEpochMilli());
